 ...
 *
 * 1km 에서 사진이 뭉치는 경우가 있긴 하나 그건 한 자리에 오래 머문 극단이고, 보통은
 * 그 배율이 「이 동네에서 뭘 봤나」를 보는 자리다 (실사용 판단).
 *
 * ⚠️ 2500 은 막대 눈금 사이에 있다. ScaleControl 은 1·2·3·5×10ⁿ 만 표시하므로
 *    「2.5km」라는 눈금은 없고, 막대가 2km 를 가리키는 구간(실제 2000~3000m) 한가운데서
 *    갈린다 — 같은 「2km」 표시에서도 조금 더 확대하면 사진이 돌아온다. 눈금과 딱 맞추려면
 *    2000(2km 표시부터 숨김)이나 3000(3km 표시부터 숨김)이어야 한다.
 */
const SHOT_MAX_M = 2500
