/**
 * 사진 행 삭제 — 사진 추가의 「전부 취소」가 부르는 경로.
 * 바이트가 끝내 도착하지 않은 행이 남으면 깨진 이미지가 되므로 조용한 실패다.
 * 사진이 0장이 된 포인트도 같이 지운다.
 *
 * 🔴 기록의 기간(started_at/ended_at)은 여기서 다시 계산하지 않는다. 편집 화면에서
 *    손으로 정한 값일 수 있어서다. 사진을 «붙이는» 쪽(posts/[slug]/photos.post.ts)은
 *    다시 계산하므로, 붙였다가 전부 취소하면 넓어진 기간이 그대로 남을 수 있다.
 *
