...
### 포인트 위치 지정 (2026-09-09)

대표 사진 위치를 기본으로 사용한다. 사진 한 장인 포인트는 위치 버튼을 흐리게 표시하고 비활성화한다.
모바일 도구 줄의 폭을 늘리지 않도록 일괄 지정은 포인트 편집 단계의 상단 기록 메뉴에 둔다.
데스크톱도 같은 메뉴 항목을 공유하고, 실제 이동 대상 수와 이미 적용된 상태를 표시한다.
일괄 지정은 로컬 초안이며 기존 저장·변경 취소 흐름으로 처리한다.
