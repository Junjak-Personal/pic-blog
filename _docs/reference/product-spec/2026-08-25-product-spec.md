...
- 새 포인트의 `point.lat/lng` 기본값은 **대표 사진 위치**다. 최초 업로드·사진 추가의 새 포인트·반경 재계산은 촬영 시각 순 첫 사진, 수동 분리는 현재 첫 사진을 사용한다.
- 클러스터 합류 판정에는 계속 평균 중심을 사용하며, 묶기가 끝난 뒤 대표 사진 위치로 확정한다. 사진 추가로 기존 포인트에 합류할 때는 저장 위치를 보존한다.
- 편집에서는 대표 사진 위치와 사진 평균을 고를 수 있다. 사진 한 장인 포인트는 그 사진 위치를 사용하며 위치 지정 버튼은 비활성화한다.
- 기존 저장 위치는 유지한다. 대표 사진 위치를 쓰던 포인트는 편집 중 대표 사진 변경을 따라가며, 변경된 위치는 저장할 때 반영한다.
- 2단계의 상단 기록 메뉴에 **모두 대표 사진 위치로**를 둔다. 대상 수를 표시하고, 초안에만 적용하므로 저장 전 변경 취소가 가능하다.
...
- 지터 격자 → 타원 충돌 완화 6회
- 카드 크기는 장수에 반비례한다 (`1/√n`). 겹침은 10–20% 를 넘기지 않는다
- 필드: 데스크탑 716×632 / 모바일 390×430 을 SSR 폴백으로 쓰고, 실제 배치는 컨테이너 크기를 잰다
