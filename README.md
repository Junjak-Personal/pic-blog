...
### iOS PWA 상단이 흐리게 보일 때

홈 화면에 추가한 앱에서 상단바의 글자·로고가 뿌옇게 번져 보이는 경우가 있다.
같은 페이지를 Safari 로 열면 멀쩡하다.

**앱이나 CSS 문제가 아니다.** iOS 가 상태바 아래에 반투명 레이어를 그리면서 그 밑의
콘텐츠가 비쳐 보이는 것이고, 기기의 투명도 설정에 달려 있다.

고치는 법 — iPhone에서:

1. **설정 → 손쉬운 사용 → 디스플레이 및 텍스트 크기**
2. **투명도 줄이기(Reduce Transparency)** 를 켠다
3. 홈 화면의 앱을 완전히 닫았다가 다시 연다

여전하면 기기를 재시동한다. iOS 버전에 따라 상태바 아래 구분선이나 반투명 처리가
다르게 나타날 수 있다.

> 한때 이 현상을 앱 쪽 문제로 보고 상단바를 몇 px 내리는 대응을 넣었다가 되돌렸다.
> 기기 설정에서 나는 현상이라 레이아웃으로 맞추면 설정이 다른 기기에서 어긋난다.
